...
public class MonthlyHealthDataProcessor implements ItemProcessor<User, HealthData> {
    ...
    public HealthData process(User user) throws Exception {
        ...
        List<HealthData> weekData = healthDataRepository.findByUserIdAndCreatedDateBetweenAndDataType(user.getId(), startDate, endDate, DataType.WEEKAVG); //전달의 1일부터 전달의 마지막날까지의 데이터
        if(weekData.isEmpty()) {
        ...
        String period = String.format("%d년 %d월",year,month);
        ...
        user.getMyHealthData().add(monthAvg); //
        return monthAvg;
    }
}
