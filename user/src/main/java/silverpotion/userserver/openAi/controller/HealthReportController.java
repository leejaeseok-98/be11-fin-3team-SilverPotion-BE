...
public class HealthReportController {
    ...
    //1. 헬스리포트 생성 및 실시간 조회
    @GetMapping("/gptqna")
    public Mono<ResponseEntity<?>> reportCreate(@RequestHeader("X-User-Id")String loginId) {
//mono를 리턴하는 웹플럭스 방식이기 때문에 아래처럼 리턴한다. .map은 mono안에 있는 진짜 값을 뽑아냄.(여기선 Mono<String>이니까 String타입의 내용을 꺼내서 우리가 원하는ResponseEntity 방식으로 리턴!)
    ...
    public ResponseEntity<?> pastReport(@RequestHeader("X-User-Id")String loginId, @PathVariable String selectedDate){
    ...
    public ResponseEntity<?> pastReportOfDep(@RequestHeader("X-User-Id")String loginId, @RequestBody HealtReportOfDepReqDto reqDto){
